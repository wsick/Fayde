...
define(["require", "exports"], function(require, exports) {
    var Format = Fayde.Localization.Format;

    function run() {
        QUnit.module("Format Tests");

        function ft(format, num, expected) {
            strictEqual(Format(format, num), expected);
        }
        function fdt(format, dt, expected) {
            strictEqual(Format(format, dt), expected);
        }
        function ftt(format, ts, expected) {
            strictEqual(Format(format, ts), expected);
        }
        function testCustomTimeSpan(msg, ts, format, expected) {
            strictEqual(Format(format, ts), expected, msg);
        }
        function testCustomDateTime(dt, format, expected) {
            strictEqual(Format(format, dt), expected, format);
        }

        var dt = new DateTime(1397133466779);
        test("DateTime: Short date", function () {
            fdt("{0:d}", dt, "4/10/2014");
        });
        test("DateTime: Long date", function () {
            fdt("{0:D}", dt, "Thursday, April 10, 2014");
        });
        test("DateTime: Full date/time (short time)", function () {
            fdt("{0:f}", dt, "Thursday, April 10, 2014 8:37 AM");
        });
        test("DateTime: Full date/time (long time)", function () {
            fdt("{0:F}", dt, "Thursday, April 10, 2014 8:37:46 AM");
        });
        test("DateTime: General date/time (short time)", function () {
            fdt("{0:g}", dt, "4/10/2014 8:37 AM");
        });
        test("DateTime: General date/time (long time)", function () {
            fdt("{0:G}", dt, "4/10/2014 8:37:46 AM");
        });
        test("DateTime: Month/day", function () {
            fdt("{0:m}", dt, "April 10");
            fdt("{0:M}", dt, "April 10");
        });
        test("DateTime: RFC123", function () {
            fdt("{0:r}", dt, "Thu, 10 Apr 2014 12:37:46 GMT");
            fdt("{0:R}", dt, "Thu, 10 Apr 2014 12:37:46 GMT");
        });
        test("DateTime: Sortable date/time", function () {
            fdt("{0:s}", dt, "2014-04-10T08:37:46");
        });
        test("DateTime: Short time", function () {
            fdt("{0:t}", dt, "8:37 AM");
        });
        test("DateTime: Long time", function () {
            fdt("{0:T}", dt, "8:37:46 AM");
        });
        test("DateTime: Universal sortable date/time", function () {
            fdt("{0:u}", dt, "2014-04-10 08:37:46Z");
        });
        test("DateTime: Universal full date/time", function () {
            fdt("{0:U}", dt, "Thursday, April 10, 2014 8:37:46 AM");
        });
        test("DateTime: Year month", function () {
            fdt("{0:y}", dt, "April, 2014");
            fdt("{0:Y}", dt, "April, 2014");
        });
        test("DateTime: Custom", function () {
        ...
        test("TimeSpan: Constant", function () {
        ...
        test("Number: Exponential", function () {
        ...
        test("Number: Fixed-point", function () {
        ...
        test("Number: Hexadecimal", function () {
            ft("{0:x}", -1, "ff");
            ft("{0:X}", -1, "FF");
