...
class DemoNflDataProvider {
    ...
    private _teams: Team[];
    private _fantasyPlayers: Fayde.Collections.ObservableCollection<FantasyPlayer>;

    get FantasyTeams(): FantasyTeam[]{
        if (this._fantasyTeams) return this._fantasyTeams;
    ...
    get DraftSpots(): Fayde.Collections.ObservableCollection<Round> {
        var rounds: Fayde.Collections.ObservableCollection<Round> = new Fayde.Collections.ObservableCollection<Round>();
    ...
    get FantasyPlayers(): Fayde.Collections.ObservableCollection<FantasyPlayer> {
        if (this._fantasyPlayers) return this._fantasyPlayers;
        this._fantasyPlayers = new Fayde.Collections.ObservableCollection<FantasyPlayer>();
        var fp = [
            {
        ...
        for (var i = 0; i < fp.length; i++) {
            ...
            if (stats) {
                for (var j = 0; j < stats.length; j++) {
                    ...
                    if (j === fp[i]["Stats"].length - 1) fantasyPlayer.Projected = s;
                    else fantasyPlayer.Stats.push(s);
                }
            }
    ...
    get FantasyPositions(): FantasyPosition[] {
